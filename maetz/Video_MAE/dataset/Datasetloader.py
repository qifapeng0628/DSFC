    ...
    def __getitem__(self, index):
        ...
        for i,fileinput in enumerate(fileinputs_s):
            ...
            if self.datamodal == 'rgb':
                ...
                img = cv2.imread(fileinput)[:, :, [2, 1, 0]] #最后一个通道与BGR转成RGB
                w, h, c = img.shape  #这里有点问题可能是h,w,c
                if w < 226 or h < 226: #这个if应该不会被执行，因为w=856，h=480
                    d = 226. - min(w, h)
                    sc = 1 + d / min(w, h)
                    img = cv2.resize(img, dsize=(0, 0), fx=sc, fy=sc)
                img = (img / 255.) * 2 - 1 #将img里面的所有数据变为-1~1之间
            elif self.datamodal == 'flow' or self.datamodal == 'flownet':
            ...
            #     sc = 1 + d / min(w, h)
            #     img = cv2.resize(img, dsize=(0, 0), fx=sc, fy=sc)
            img = cv2.resize(img, (224, 224)) #将图片变为224*2224
...
def frame2clip_anolabel(framelabels=None):
    ...
    if np.count_nonzero(np.asarray(frame_ano_label,dtype='int')) >= len(frame_ano_label) - np.count_nonzero(np.asarray(frame_ano_label,dtype='int')):
        #如果16个帧标签中有一半以上的是1,那么这个片段的标签就为1，反之亦然
        clip_ano_label = 1
    else:
        clip_ano_label = 0
...
if __name__ == '__main__':
    datamodal = 'rgb'
    origin_filelist = '.\\shanghaitech\\i3d\\{}_list.txt'.format(datamodal)
    origin_labellist = '.\\shanghaitech\\i3d\\label.txt'
    trainfile_list = '.\\{}\\{}\\{}_list_numJoints.txt'.format('shanghaitech','i3d', datamodal)
    trainlabel_list = '.\\{}\\{}\\trainlabel_numJoints.txt'.format('shanghaitech','i3d')
    numJoints = 16
    ...
    train_dataset = trainDataset(list_file=trainfile_list,
                                 GT_file=trainlabel_list, transform=None, cliplen=numJoints, datamodal='rgb')
    #
